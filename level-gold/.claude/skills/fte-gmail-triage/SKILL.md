...
1. Write `AI_Employee_Vault/Pending_Approval/APPROVAL_email_reply_<message_id[:8]>.md` with:
...
- Write `AI_Employee_Vault/Needs_Action/ATTACHMENT_EXTRACT_<message_id[:8]>.md`:
```yaml
---
...
b) **Write holding reply to /Pending_Approval** — `AI_Employee_Vault/Pending_Approval/APPROVAL_email_reply_<message_id[:8]>.md`:
```yaml
---
type: email_reply
action_id: <message_id[:8]>
...
c) **Write an ODOO trigger file** to `AI_Employee_Vault/Needs_Action/ODOO_<email_stem>.md`:
```yaml
---
type: odoo_workflow
source_email: <EMAIL_*.md filename>
...
priority: high
approval_file: Pending_Approval/APPROVAL_email_reply_<id>.md
odoo_trigger_file: Needs_Action/ODOO_<email_stem>.md
triaged_at: <ISO timestamp>
processed_by: fte-gmail-triage
