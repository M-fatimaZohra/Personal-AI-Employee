...
Read `AI_Employee_Vault/Needs_Action/<ATTACHMENT_EXTRACT_*.md>` passed as `$ARGUMENTS`.
...
- Write `AI_Employee_Vault/Pending_Approval/APPROVAL_attach_review_<message_id[:8]>.md`:
  ```yaml
  ---
  type: manual_review
  from: <sender>
...
Write `AI_Employee_Vault/Pending_Approval/APPROVAL_invoice_request_<message_id[:8]>.md`:
