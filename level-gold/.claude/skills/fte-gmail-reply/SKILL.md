...
- If `$ARGUMENTS` is provided: read `AI_Employee_Vault/Needs_Action/<argument>`
- Otherwise: find the oldest EMAIL_*.md with `action: reply_needed` in `AI_Employee_Vault/Needs_Action/`
...
Write `AI_Employee_Vault/Pending_Approval/APPROVAL_email_reply_<message_id[:8]>.md` with:
