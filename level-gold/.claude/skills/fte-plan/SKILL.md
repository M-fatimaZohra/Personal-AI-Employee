...
- If `$ARGUMENTS` provided, read `AI_Employee_Vault/Needs_Action/$ARGUMENTS`
- Otherwise read the most recent unplanned high-priority item in Needs_Action
...
- Check `AI_Employee_Vault/Plans/PLAN_<item_stem>*.md` — if found, report it and stop
...
Write to `AI_Employee_Vault/Plans/PLAN_<item_stem>.md`:
...
- If `requires_approval: true`:
  ...
  - If NOT in Approved/: write `AI_Employee_Vault/Pending_Approval/APPROVAL_odoo_<plan_id>.md` with invoice details, then update plan frontmatter `status: awaiting_approval` — **STOP K3 here, do not re-create the file on next tick**
...
- If `requires_approval: true`: write `AI_Employee_Vault/Pending_Approval/APPROVAL_email_<plan_id>.md`
