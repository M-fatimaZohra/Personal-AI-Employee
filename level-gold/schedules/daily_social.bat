...
:: Primary path: invoke fte-social-post skill via CCR Code CLI
where ccr >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo [%DATE% %TIME%] Invoking /fte-social-post all via ccr code CLI >> "%LOG_FILE%"
    ccr code -p "/fte-social-post all" >> "%LOG_FILE%" 2>&1
) else (
