...
:: Primary path: invoke fte-linkedin-draft skill via CCR Code CLI
where ccr >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo [%DATE% %TIME%] Invoking /fte-linkedin-draft via ccr code CLI >> "%LOG_FILE%"
    ccr code -p "/fte-linkedin-draft" >> "%LOG_FILE%" 2>&1
) else (
