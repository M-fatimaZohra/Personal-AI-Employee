...
# claude is installed as claude.CMD on Windows — requires shell to execute
...
KNOWN_ACTION_TYPES = {"email_reply", "linkedin_post", "whatsapp_reply"}
...
class ApprovalWatcher(BaseWatcher):
    ...
    def __init__(
        ...
        # Wrap MCP dispatch with exponential backoff retry
        self._send_via_mcp = retry_with_backoff(
            max_tries=3,
            base_delay=2.0,
            max_delay=30.0,
            logs_path=self.logs,
    ...
    def _process_approved(self, path: Path) -> Path:
        ...
        # All other types: dispatch via MCP (fte-approve skill via claude subprocess)
        try:
            self._send_via_mcp(
                {"type": action_type, "frontmatter": fm, "content": text, "path": path}
            )
            log_action(
                self.logs, "approval_executed", "ApprovalWatcher",
                source=path.name, result="success",
                details=f"Dispatched: {action_type}",
            )
            done_path = self._move_to_done(path, "executed")
        except Exception as e:
            log_action(
                self.logs, "approval_dispatch_error", "ApprovalWatcher",
                source=path.name, result="error", details=str(e),
            )
            done_path = self._move_to_done(path, "dispatch_failed")
    ...
    # ------------------------------------------------------------------
    # MCP dispatch — Option B: fte-approve skill via claude subprocess
    # ------------------------------------------------------------------

    def _mcp_send_impl(self, action_data: dict) -> None:
        """Dispatch an approved action via the fte-approve Claude skill.

        Spawns `claude --print "/fte-approve <filename>"` as a blocking subprocess
        so that `retry_with_backoff` can detect failures and retry automatically.

        The fte-approve skill handles all MCP tool calls (send_email, draft_email)
        and archives the file to /Done on success.  This mirrors the orchestrator's
        `check_approved()` pattern but runs synchronously to support retries.

        Args:
            action_data: Dict with keys:
                type        — action type (e.g. "email_reply")
                frontmatter — parsed frontmatter dict
                content     — full file text
                path        — Path object of the approval file in /Approved

        Raises:
            RuntimeError: if `path` is missing or if the subprocess exits non-zero,
                          triggering `retry_with_backoff` for up to 3 attempts.
        """
        path: Path | None = action_data.get("path")
        if path is None:
            raise RuntimeError(
                "_mcp_send_impl: 'path' key missing from action_data — cannot dispatch"
            )

        _flag_dir = Path(__file__).parent / ".state"
        _flag_dir.mkdir(exist_ok=True)
        _flag = _flag_dir / f"dispatch_aw_{int(time.time())}.flag"
        _flag.write_text("1")
        try:
            result = subprocess.run(
                ["claude", "--print", f"/fte-approve {path.name}"],
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"fte-approve skill failed (exit={result.returncode}): "
                f"{result.stderr[:200]}"
            )
