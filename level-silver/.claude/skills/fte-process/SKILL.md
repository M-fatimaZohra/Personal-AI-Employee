...
1. **Select the item**:
   - If `$ARGUMENTS` is provided, find the matching file in `AI_Employee_Vault/Needs_Action/`
...
4. **Check for an existing plan** in `AI_Employee_Vault/Plans/`:
...
7. **For items needing a plan**, write `AI_Employee_Vault/Plans/PLAN_<item_stem>.md`:
   ```yaml
   ---
   type: plan
...
8. **For external actions** (email send, post, payment), write an approval file:
   `AI_Employee_Vault/Pending_Approval/APPROVAL_<action>_<id>.md`
