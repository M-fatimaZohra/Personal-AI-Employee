...
2. **Check for existing briefing**: Glob `AI_Employee_Vault/Plans/**/BRIEFING_<date>*.md` (search recursively).
...
3. **Gather intelligence** (read all sources):
   ...
   **a) Pending items in Needs_Action** (scan recursively with `**/*.md`):
   ...
   **b) Pending Approvals** (scan recursively with `**/*.md`):
   ...
   **c) Active Plans** (scan recursively with `**/*.md`):
...
4. **Write the briefing** to `AI_Employee_Vault/Plans/general/BRIEFING_<YYYY-MM-DD>.md`:
