...
1. **Check for today's existing draft**: Glob `AI_Employee_Vault/Plans/**/LINKEDIN_DRAFT_<today-date>*.md` (search recursively).
...
5. **Write the draft file** to `AI_Employee_Vault/Plans/social/LINKEDIN_DRAFT_<YYYY-MM-DD>.md`:
   ```yaml
   ---
   type: linkedin_post
   status: draft
