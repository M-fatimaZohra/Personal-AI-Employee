...
2. **List all files** in `AI_Employee_Vault/Needs_Action/` using Glob (`**/*.md`) to scan domain subdirectories recursively. If `$ARGUMENTS` is provided, filter to files matching that string.
