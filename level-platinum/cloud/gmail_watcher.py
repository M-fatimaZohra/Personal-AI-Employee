...
class GmailWatcher(BaseWatcher):
    ...
    def __init__(
        ...
        # Domain-based folder: emails go to Needs_Action/email/
        self.needs_action = self.vault_path / "Needs_Action" / "email"
        self.needs_action.mkdir(parents=True, exist_ok=True)

        # Paths are resolved relative to level-platinum/.secrets/ so both
        # cloud and local agents can share the same credentials
    ...
    def _save_processed_id(self, message_id: str) -> None:
        ...
        # Don't save IDs in DRY_RUN mode - keep it truly non-destructive
        import os
