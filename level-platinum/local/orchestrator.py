"""orchestrator.py — Platinum Tier Local Agent (Full Execution Mode)
...
Local Agent Responsibilities:
    ...
    4. Claim-by-move       — atomically claim files to In_Progress/local/,
                             skip files already claimed by cloud agent.
    5. Dashboard merge     — merge Updates/cloud_status.md into Dashboard.md.
    6. Plan awareness      — detect active Plan files with unchecked steps
                             and re-prompt Claude to continue (Ralph Wiggum).
    7. Dashboard sync      — call update_dashboard() on every tick so the
                             Obsidian UI always reflects live state.
    8. Odoo health check   — every ODOO_HEALTH_INTERVAL ticks, probe Odoo
                             JSON-RPC and update service_health in Dashboard.
    9. Graceful shutdown   — SIGINT / SIGTERM stop all watchers + subprocesses.
...
class Orchestrator:
    ...
    def __init__(
        ...
        # Platinum Tier — claim-by-move directories
        ...
        # Ensure Platinum directories exist
    ...
    def try_claim_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            log_action(
                ...
                destination=f"In_Progress/local/{filepath.name}",
                result="success",
            )
            return True
        except FileNotFoundError:
            # File already claimed by cloud agent
            return False
    ...
    def is_claimed_by_cloud(self, filename: str) -> bool:
        """Check if a file is already claimed by the cloud agent."""
        return (self.in_progress_cloud / filename).exists()

    # ------------------------------------------------------------------
    # Platinum Tier — Dashboard merge (single-writer rule)
    # ------------------------------------------------------------------

    def merge_cloud_status(self) -> None:
        """Merge Updates/cloud_status.md into Dashboard.md, then delete Updates/.

        This implements the single-writer rule: cloud writes to Updates/,
        local merges into Dashboard.md. Prevents Git merge conflicts.
        """
        cloud_status_file = self.updates / "cloud_status.md"

        if not cloud_status_file.exists():
            return

        try:
            # Cloud status is already parsed and merged by dashboard_updater.py
            # when we call update_dashboard() - it reads Updates/cloud_status.md
            # and includes it in the "## Cloud Agent" section automatically.

            # After Dashboard.md is updated with cloud data, delete the source file
            # to prevent stale status from being re-merged on next tick
            cloud_status_file.unlink()
            ...
            log_action(
                ...
                "LocalOrchestrator",
                result="success",
                details="Cloud status merged into Dashboard.md and Updates/ cleared",
            )

        except Exception as e:
            log_action(
                self.logs,
                "cloud_status_merge_failed",
                "LocalOrchestrator",
                result="error",
                details=str(e),
            )
    ...
    def init_watchers(self) -> None:
        ...
        wa_session = Path(__file__).parent.parent / ".secrets" / "whatsapp_session"
        ...
        li_session = Path(__file__).parent.parent / ".secrets" / "linkedin_session"
        ...
        fb_session = Path(__file__).parent.parent / ".secrets" / "facebook_session"
        ...
        ig_session = Path(__file__).parent.parent / ".secrets" / "instagram_session"
        ...
        tw_session = Path(__file__).parent.parent / ".secrets" / "twitter_session"
    ...
    def check_needs_action(self) -> None:
        ...
        for filepath in sorted(self.needs_action.glob("**/*.md")):
            ...
            # Platinum Tier — skip if cloud agent already claimed this file
            if self.is_claimed_by_cloud(filepath.name):
                continue

            # Platinum Tier — try to claim the file atomically
            if not self.try_claim_file(filepath):
                # Cloud agent claimed it first — skip
                continue

            # File is now in In_Progress/local/ — process from there
            claimed_path = self.in_progress_local / filepath.name

            # Skip files the skill has already handled (status written to frontmatter)
            # Auto-cleanup: move processed files out of In_Progress/local/ → Done
            if self._already_processed(claimed_path):
                ...
                if dest.exists():
                    ...
                    dest = done_dir / f"{claimed_path.stem}_{ts}{claimed_path.suffix}"
                try:
                    claimed_path.rename(dest)
                    log_action(self.logs, "needs_action_archived", "Orchestrator",
                               source=claimed_path.name, destination=dest.name, result="success",
                               details="Auto-archived processed file from In_Progress/local to Done")
                except OSError:
                    pass
                continue

            skill = self.skill_for(claimed_path.name)
            if skill is None:
                continue  # no routing rule for this prefix — skip silently

            self._dispatch(claimed_path, f"/{skill} {claimed_path.name}")
    ...
    def check_linkedin_schedule(self) -> None:
        ...
        session_dir = (_Path(__file__).parent.parent / session_raw).resolve()
    ...
    def tick(self) -> None:
        ...
        # Collect counts before dispatch for the tick log (recursive for domain subdirectories)
        needs_action_count = (
        ...
        # Platinum Tier — merge cloud status updates into Dashboard
        try:
            self.merge_cloud_status()
        except Exception as exc:
            log_action(
                self.logs, "orchestrator_error", "Orchestrator",
                result="error", details=f"merge_cloud_status: {exc}",
            )
