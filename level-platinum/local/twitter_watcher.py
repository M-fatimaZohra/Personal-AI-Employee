...
TWITTER_SESSION_DIR = (
    Path(__file__).parent.parent / os.getenv("TWITTER_SESSION_DIR", ".secrets/twitter_session")
).resolve()
...
class TwitterWatcher(BaseWatcher):
    ...
    def create_action_file(self, item: dict) -> Path:
        """Write TWITTER_<id>.md to Needs_Action/social/."""
        now = datetime.now(timezone.utc)
        filename = f"TWITTER_{item['id']}.md"
        filepath = Path(self.vault_path) / "Needs_Action" / "social" / filename
